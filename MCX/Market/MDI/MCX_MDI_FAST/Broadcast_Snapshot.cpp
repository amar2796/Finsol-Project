...
Broadcast_Snapshot::Broadcast_Snapshot(uint64_t security, uint16_t exchID, FIN::Publisher* publish) :market_depth(exchID, std::to_string(security)),
	...
	try
	{
		...
		for (auto entry : group)
		{
			...
			case 0: //Bid
				...
				market_depth.insertBuyDepth((FIN::PRICE)entry.get_MDEntryPx(), (FIN::QUANTITY)entry.get_MDEntrySize());
				break;
			case 1: //Ask
				//LOG_INFO << "PROCESS ASK SNAP";
				market_depth.insertSellDepth((FIN::PRICE)entry.get_MDEntryPx(), (FIN::QUANTITY)entry.get_MDEntrySize());
				break;
			case 2://Trade
			{
			...
			case 8://open interest ( C )			
				...
				market_depth.setOpenInterest(entry.get_MDEntrySize());
				break;
			case 9://Trade Volume
...
void Broadcast_Snapshot::process_Increment_Update(MDIFastTemplates_1_1::DepthIncremental_cref::MDIncGrp_element_cref& entry)
{
	try
	{
		...
		{
		case 0:
			market_depth.updateBuyDepth((FIN::PRICE)entry.get_MDEntryPx(), (FIN::QUANTITY)entry.get_MDEntrySize());
			break;
		case 1:
			market_depth.updateSellDepth((FIN::PRICE)entry.get_MDEntryPx(), (FIN::QUANTITY)entry.get_MDEntrySize());
			break;
...
void Broadcast_Snapshot::process_Increment_Update_Overlay(MDIFastTemplates_1_1::DepthIncremental_cref::MDIncGrp_element_cref& entry)
{
	try
	{
		...
		{
		case 0:
			market_depth.updateBuyDepth_Overlay((FIN::PRICE)entry.get_MDEntryPx(), (FIN::QUANTITY)entry.get_MDEntrySize(), entry.get_MDPriceLevel().value());
			break;
		case 1:
			market_depth.updateSellDepth_Overlay((FIN::PRICE)entry.get_MDEntryPx(), (FIN::QUANTITY)entry.get_MDEntrySize(), entry.get_MDPriceLevel().value());
			break;
