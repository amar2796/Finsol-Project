...
namespace FIN {
	...
	public:
		...
		inline uint64_t getOmsTerminalINfo(uint64_t config_terminalInfo)
		{

			auto oms_terminal = get(FIN::TERMINAL_TAG);

			if (oms_terminal != "" && oms_terminal != "0") {
				return std::stoll(oms_terminal);
			}

			return config_terminalInfo;
		}
	...
	public:
		...
		inline void getOmsString(std::string& out)
		{
			_itr = _mapKeyValues.begin();
			_itrEnd = _mapKeyValues.end();
			updateString(out, Fix_BeginString);
			updateString(out, Fix_MsgType);
			updateString(out, Fix_SenderCompID);
			updateString(out, Fix_SendTime);
			...
			updateString(out, Fix_LeavesQty);
			updateString(out, Fix_OrigClOrdID);
			updateString(out, Fix_ExecRestatementReason);


			updateString(out, Fix_NumberFill);
			updateString(out, Fix_FillPx);
			updateString(out, Fix_Fill_Qty);
			updateString(out, Fix_FillMatchId);
			updateString(out, Fix_FillExecId);

			updateString(out, Fix_CrossIndicator);
			updateString(out, Fix_TransactioDelayIndicator);
		...
		inline void insertField(const std::string& key, const char val)
		{
			_mapKeyValues[key] = val;
		}
	private:
