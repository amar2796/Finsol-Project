...
void UDP_Sender::init(string ip, int port)
{
	this->ip = ip;
	this->port = port;
	std::cout << "[INFO ]UDP PUBLISHER : IP " << ip << " PORT " + std::to_string(port) << std::endl;
	LOG_INFO << "UDP PUBLISHER : IP " << ip << " PORT " + std::to_string(port);
...
void UDP_Sender::initializeUdpSocket()
{
	...
	else
	{
		...
		{
			std::cout << "[INFO ]UDP PUBLISHER IS READY TO SEND " << std::endl;
			LOG_INFO << "UDP PUBLISHER IS READY TO SEND ";
...
bool UDP_Sender::sendToOMS(char* msg, size_t len, short orderStaus, uint64_t orderId)
{
	if (_isConnected)
	{
		...
		if (sendto(SendingSocket, msg, len, 0, (struct sockaddr*)NULL, sizeof(ReceiverAddr)) <= 0)
		{
			...
			LOG_INFO << "Data sent error " + std::to_string(WSAGetLastError());
		}
	...
	else
	{
		...
		LOG_INFO << "CLIENT IS NOT CONNECTED " + std::to_string(WSAGetLastError());
