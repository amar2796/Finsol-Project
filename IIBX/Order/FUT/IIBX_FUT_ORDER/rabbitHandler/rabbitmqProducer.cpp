...
namespace FIN {
	...
	bool RabbitMqProducer::sendToOMS(const std::string& _message)
	{
	...
	bool RabbitMqProducer::sendToOMS(char* _message, size_t len)
	{
		amqp_bytes_t msg;
		msg.bytes = _message;
		msg.len = len;
		...
		props.delivery_mode = 2;
		return amqp_basic_publish(_rp->getConnectionState(), _channel_id, amqp_cstring_bytes(_exchangeName.c_str()), amqp_cstring_bytes(_routingKey.c_str()), 1, 0, &props, msg);		
