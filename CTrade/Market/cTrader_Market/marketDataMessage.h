...
namespace FIN
{
	...
	const static int DEPTH_SIZE_COMMON = 20;
	const static int DEPTH_SIZE_5 = 5;
	const static int DEPTH_SIZE_10 = 10;
	struct BroadCastMsgHeader
	{
	public:

		BroadCastMsgHeader(const UNSIGNED_INT_16 msgLength, const SIGNED_INT_8 msgType)
	...
	public:
		...
		MarketData( const std::string& secId)
			:BroadCastMsgHeader(sizeof(MarketData), BroadCastMsgType_MarketData),
			_ltp(0),
			_volume(0)
		{
			memset(_secId, 0, 25);
			memcpy(_secId, secId.c_str(), secId.length());
			deleteCompleteDepth();
		}
		/*
	...
	private:
		...
		Depth         _buyDepth[DEPTH_SIZE_COMMON];
		Depth         _sellDepth[DEPTH_SIZE_COMMON];
		PRICE         _ltp;
	...
	public:
		...
		OHLC( const std::string& secId)
			:BroadCastMsgHeader(sizeof(OHLC), BroadCastMsgType_OHLC),
			_openPrice(0),
			_highPrice(0),
			_closePrice(0),
			_lowPrice(0)
	...
	public:
		DPR(const UNSIGNED_INT_16 exchange, const std::string& secId)
			:BroadCastMsgHeader(sizeof(DPR), BroadCastMsgType_DPR),
			_upperLimit(0),
			_lowerLimit(0)
		{
			memset(_secId, 0, 25);
	...
	public:
		MarketNotice(const UNSIGNED_INT_16 exchange)
			:BroadCastMsgHeader(sizeof(MarketNotice), BroadCastMsgType_MarketNotice),
	...
	public:
		...
		SymbolNotice( const std::string& secId)
			:BroadCastMsgHeader(sizeof(SymbolNotice), BroadCastMsgType_SymbolNotice),
...
*/
	...
	public:
		...
		ModuleConnectivity(const UNSIGNED_INT_16 exchange)
			:BroadCastMsgHeader(sizeof(ModuleConnectivity), BroadCastMsgType_Connectivity),
	...
	public:
		SessionInfo(const UNSIGNED_INT_16 exchange)
			:BroadCastMsgHeader(sizeof(SessionInfo), BroadCastMsgType_Connectivity),
	...
	// 5 depth market
	struct MarketData_5Depth : public BroadCastMsgHeader
	{
	public:
		//constructer with string of secId.
		MarketData_5Depth( const std::string& secId)
			:BroadCastMsgHeader(sizeof(MarketData_5Depth), BroadCastMsgType_MarketData),
			_ltp(0),
			_volume(0)
		{
			memset(_secId, 0, 25);
		...
		void deleteBuyDepth(const PRICE);
		void deleteSellDepth(const PRICE);
		void insertBuyDepth(const PRICE, const int64_t);
		void insertSellDepth(const PRICE, const int64_t);
		void updateBuyDepth(const PRICE, const int64_t);
		void updateSellDepth(const PRICE, const int64_t);
		bool getPositionForBuyPrice(const PRICE, UNSIGNED_INT_16&);
		bool getPositionForSellPrice(const PRICE, UNSIGNED_INT_16&);
		void deleteCompleteDepth();

		void deleteBuyDepthViaId(const PRICE, const UNSIGNED_INT_16, const SIZE);
		void deleteSellDepthViaId(const PRICE, const UNSIGNED_INT_16, const SIZE);
	...
	// 10 depth market
	//struct MarketData_10Depth : public BroadCastMsgHeader
	//{
	//public:
	//	//constructer with string of secId.
	//	MarketData_10Depth(const UNSIGNED_INT_16 exchange, const std::string& secId)
	//		:BroadCastMsgHeader(sizeof(MarketData_10Depth), BroadCastMsgType_MarketData),
	//		_ltp(0),
	//		_volume(0)
	//	{
	...
	//	//public Functions
	//	void setDepthCount(SIGNED_INT_16 depth);
	//	void applyTrade(const PRICE);
	//	void deleteBuyDepth(const PRICE);
	//	void deleteSellDepth(const PRICE);
	//	void insertBuyDepth(const PRICE, const UNSIGNED_INT_16);
	...
	namespace BROADCAST
	{
		const static int DEPTH_SIZE_DEFAULT = 5;
		//const static int DEPTH_SIZE = 5;
		struct MarketData : public BroadCastMsgHeader
		{
			void copyMD(FIN::MarketData& md, short count)
			{
				this->depthCount = count;
				this->setUpdateTime(md.getUpdateTime());
				memset(_secId, 0, 25);
				memcpy(_secId, md.getSecIdPtr(), md.getSecIdLength());
			...
			MarketData(FIN::MarketData& md) :BroadCastMsgHeader(sizeof(MarketData), md.getMsgType())
			{
				this->setUpdateTime(md.getUpdateTime());
		...
		struct MarketData_default : public BroadCastMsgHeader
		{
			...
			char _secId[25];
			Depth buyDepth[DEPTH_SIZE_DEFAULT];
			Depth sellDepth[DEPTH_SIZE_DEFAULT];
		...
		public:
			...
			MarketData_default( const std::string& secId);
			void print() const;
