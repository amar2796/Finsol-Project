...
#define LOG_INFILE( ARG )\
	LOG_INFO<<ARG << std::endl

#define FIN_DEVLOG( ARG )\
	std::cout  << log_getCurrentTime() << ARG<< std::endl
...
#define FIN_ERROR( ARG )\
	std::cerr  << log_getCurrentTime() << ARG <<std::endl
