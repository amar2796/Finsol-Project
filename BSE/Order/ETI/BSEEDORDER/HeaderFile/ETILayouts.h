 ...
 *
 *  BUILD NUMBER:        700.001.854-700001850.45
 *
 *  DESCRIPTION:
 *
...
#define ETI_BUILD_NUMBER      "700.001.854-700001850.45"
...
#define	TID_SESSION_REGISTRATION_REQUEST                 10053		// < SessionRegistrationRequest (Session Registration Request)
#define	TID_SESSION_REGISTRATION_RESPONSE                10054		// < SessionRegistrationResponse (Session Registration Response)
...
// DataType AllowTLS
#define LEN_ALLOW_TLS                                    1
#define ENUM_ALLOW_TLS_CHECK_TLS                         "Y"
#define ENUM_ALLOW_TLS_CHECK_TLS_CHAR                    'Y'
#define ENUM_ALLOW_TLS_SKIP_TLS                          "N"
#define ENUM_ALLOW_TLS_SKIP_TLS_CHAR                     'N'
...
#define ENUM_EXEC_RESTATEMENT_REASON_MARKET_ORDER_OUT_OF_RANGE 265
...
// DataType InitializationVector
#define LEN_INITIALIZATION_VECTOR                        16
...
#define ENUM_QUOTE_ENTRY_REJECT_REASON_CLIENT_CODE_NOT_ALLOWED_TO_TRADE_IN_TRADING_RESTRICTION_WINDOW_PIT 155
#define ENUM_QUOTE_ENTRY_REJECT_REASON_DEACTIVATED_VIDE_SEBI_CIR_DTD_29_07_22 156
...
#define ENUM_SEGMENT_INDICATOR_EGR                       16
...
// DataType Status
#define LEN_STATUS                                       1
#define ENUM_STATUS_REJECT                               "N"
#define ENUM_STATUS_REJECT_CHAR                          'N'
#define ENUM_STATUS_ACCEPT                                "Y"
#define ENUM_STATUS_ACCEPT_CHAR                           'Y'
...
typedef struct
{
    ...
    {
        return "H-START" + to_string(BodyLen) + PIPE + to_string(TemplateID) +
            PIPE + NetworkMsgID + PIPE + Pad2 + PIPE + "H-END";
    }
...
typedef struct
{
    ...
    std::string getString()
    {
        return  "H-START " + std::to_string(BodyLen) + PIPE +
            std::to_string(TemplateID) + PIPE +
            Pad2 + PIPE + "H-END";
    }
...
typedef struct
{
    ...
    string getString()
    {
        return to_string(RequestTime) + PIPE + to_string(RequestOut) + PIPE + to_string(TrdRegTSTimeIn) + PIPE +
            to_string(TrdRegTSTimeOut) + PIPE + to_string(ResponseIn) + PIPE + to_string(SendingTime) + PIPE
            + to_string(MsgSeqNum) + PIPE + to_string(LastFragment) + PIPE + Pad3 + PIPE;
    }
...
typedef struct
{
    ...
    uint8_t OrdType;
    uint8_t TimeInForce;
    char ClientCode[LEN_CLIENT_CODE];
    uint8_t Triggered;
...
typedef struct
{
    ...
    string getString()
    {
        return to_string(TrdRegTSTimeOut) + PIPE + to_string(SendingTime) + PIPE + to_string(ApplSubID) + PIPE
            + to_string(PartitionID) + PIPE + ApplMsgID + PIPE + to_string(ApplID) + PIPE + to_string(ApplResendFlag) + PIPE +
            to_string(LastFragment) + PIPE + Pad7 + PIPE;
    }
...
typedef struct
{
    ...
    string getString()
    {
        return " REQ_H-START" + to_string(MsgSeqNum) + PIPE + to_string(SenderSubID) + PIPE + "REQ_H-END";
    }
...
typedef struct
{
    ...
    string getString()
    {
        return to_string(RequestTime) + "||" +
            to_string(SendingTime) + "||" + Pad4 + "||";
    }
...
typedef struct
{
    ...
    string getString()
    {
        return MessageHeaderOut.getString() + NRResponseHeaderME.getString() + to_string(OrderID) + PIPE + to_string(ClOrdID) + PIPE + to_string(OrigClOrdID) + PIPE +
            to_string(SecurityID) + PIPE + to_string(ExecID) + PIPE +
            to_string(CumQty) + PIPE +
            to_string(CxlQty) + PIPE +

            OrdStatus + PIPE + ExecType + PIPE + to_string(ExecRestatementReason) + PIPE + to_string(ProductComplex) + PIPE + Pad3 + PIPE;
...
typedef struct
{
    ...
    string getString()
    {
        return MessageHeaderIn.getString() + RequestHeader.getString() + " BOODY-START " + to_string(OrderID) + PIPE +
            to_string(ClOrdID) + PIPE + to_string(OrigClOrdID) + PIPE + to_string(ActivityTime) + PIPE + to_string(MessageTag) + PIPE
            + to_string(MarketSegmentID) + PIPE + to_string(SimpleSecurityID) + PIPE + to_string(TargetPartyIDSessionID) + PIPE +
            to_string(RegulatoryID) + PIPE + AlgoID + PIPE + Pad4 + PIPE;
...
typedef struct
{
    ...
    uint8_t SessionMode;
    uint8_t TradSesMode;
    char SecurityKey[LEN_SECURITY_KEY];
    char InitializationVector[LEN_INITIALIZATION_VECTOR];
    char Pad6[LEN_PAD6];
} GatewayResponseT;
...
typedef struct
{
    ...
    std::string getString()
    {
        return MessageHeaderOut.getString() +
            ResponseHeader.getString() + std::to_string(PrimaryOrderID) + "||" +
            std::to_string(ClOrdID) + "||" + std::to_string(MessageTag) + "||" +
            Pad4 + "||";
...
typedef struct
{
    ...
    uint8_t DaysLeftForPasswdExpiry;
...
typedef struct
{
    ...
    string getString()
    {
        return MessageHeaderOut.getString() + NRResponseHeaderME.getString() + to_string(OrderID) + PIPE + to_string(ClOrdID) + PIPE + to_string(OrigClOrdID) + PIPE +
            to_string(SecurityID) + PIPE + to_string(ExecID) + PIPE +
            to_string(PriceMkToLimitPx) + PIPE + to_string(Yield) + PIPE + to_string(UnderlyingDirtyPrice) + PIPE +
            to_string(ActivityTime) + PIPE + to_string(Filler1) + PIPE + to_string(Filler2) + PIPE + to_string(LeavesQty) + PIPE +
            to_string(CumQty) + PIPE +
            to_string(CxlQty) + PIPE +
            to_string(Filler4) + PIPE +
            OrdStatus + PIPE + ExecType + PIPE + to_string(ExecRestatementReason) + PIPE + to_string(ProductComplex) + PIPE;
...
typedef struct
{
    ...
    string getString()
    {
        return MessageHeaderIn.getString() + RequestHeader.getString() + " BODY START " + to_string(OrderID) + PIPE + to_string(ClOrdID) + PIPE +
            to_string(OrigClOrdID) + PIPE + to_string(Price) + PIPE + to_string(StopPx) + PIPE + to_string(MaxPricePercentage) + PIPE +
            to_string(SenderLocationID) + PIPE + to_string(ActivityTime) + PIPE + to_string(Filler1) + PIPE + to_string(Filler2) + PIPE +
            to_string(MessageTag) + PIPE + to_string(OrderQty) + PIPE + to_string(MaxShow) + PIPE + to_string(ExpireDate) + PIPE
            + to_string(MarketSegmentID) + PIPE + to_string(SimpleSecurityID) + PIPE + to_string(TargetPartyIDSessionID) + PIPE +
            to_string(RegulatoryID) + PIPE + to_string(Filler4) + PIPE + PartyIDTakeUpTradingFirm + PIPE + PartyIDOrderOriginationFirm + PIPE + PartyIDBeneficiary + PIPE +
            to_string(AccountType) + PIPE + to_string(ApplSeqIndicator) + PIPE + to_string(Side) + PIPE + to_string(OrdType) + PIPE
            + to_string(PriceValidityCheckType) + PIPE + to_string(TimeInForce) + PIPE + to_string(ExecInst) + PIPE +
            to_string(TradingSessionSubID) + PIPE + to_string(TradingCapacity) + PIPE + DeltaQtyFlag + PIPE +

            +Account + PIPE + PositionEffect + PIPE + PartyIDLocationID + PIPE + CustOrderHandlingInst + PIPE + RegulatoryText + PIPE + AlgoID + PIPE + CPCode + PIPE +
            FreeText3 + PIPE + Pad4 + " BODY END ";
    }
...
typedef struct
{
    ...
    string getString()
    {
        return MessageHeaderOut.getString() + NRResponseHeaderME.getString() + to_string(OrderID) + PIPE + to_string(ClOrdID) + PIPE +
            to_string(SecurityID) + PIPE + to_string(PriceMkToLimitPx) + PIPE + to_string(Yield) + PIPE + to_string(UnderlyingDirtyPrice) + PIPE +
            to_string(ExecID) + PIPE + to_string(ActivityTime) + PIPE + to_string(Filler1) + PIPE + to_string(Filler2) + PIPE + to_string(Filler4) + PIPE +
            OrdStatus + PIPE + ExecType + PIPE + to_string(ExecRestatementReason) + PIPE + to_string(ProductComplex) + PIPE + Pad4;
...
typedef struct
{
    ...
    string getString()
    {
        return MessageHeaderIn.getString() + RequestHeader.getString() + " BODY START " +
            to_string(Price) + PIPE + to_string(StopPx) + PIPE + to_string(MaxPricePercentage) + PIPE +
            to_string(SenderLocationID) + PIPE + to_string(ClOrdID) + PIPE + to_string(Filler1) + PIPE +
            to_string(Filler2) + PIPE + to_string(MessageTag) + PIPE +
            to_string(OrderQty) + PIPE + to_string(MaxShow) + PIPE + to_string(ExpireDate) + PIPE
            + to_string(MarketSegmentID) + PIPE + to_string(SimpleSecurityID) + PIPE + to_string(RegulatoryID) + PIPE + to_string(Filler4) + PIPE
            + ContraBroker + PIPE + PartyIDOrderOriginationFirm + PIPE + PartyIDBeneficiary + PIPE +
            to_string(AccountType) + PIPE + to_string(ApplSeqIndicator) + PIPE + to_string(Side) + PIPE + to_string(OrdType) + PIPE
            + to_string(PriceValidityCheckType) + PIPE + to_string(TimeInForce) + PIPE + to_string(ExecInst) + PIPE + to_string(STPCFlag) + PIPE + to_string(TradingSessionSubID) + PIPE + to_string(TradingCapacity) + PIPE
            + Account + PIPE + PositionEffect + PIPE + PartyIDLocationID + PIPE + CustOrderHandlingInst + PIPE + RegulatoryText + PIPE + AlgoID + PIPE + CPCode + PIPE +
            FreeText3 + PIPE + " BODY END ";
    }
...
typedef struct
{
    ...
    string getString()
    {
        return MessageHeaderOut.getString() + RBCHeaderME.getString() + to_string(OrderID) + PIPE + to_string(ClOrdID) + PIPE + to_string(OrigClOrdID) + PIPE +
            to_string(SecurityID) + PIPE + to_string(ExecID) + PIPE +
            to_string(CumQty) + PIPE +
            to_string(CxlQty) + PIPE +

            OrdStatus + PIPE + ExecType + PIPE + to_string(ExecRestatementReason) + PIPE + to_string(ProductComplex) + PIPE + Pad2 + PIPE;
...
// Message:     SessionRegistrationRequest
// TemplateID:  10053
// Alias:       Session Registration Request
// FIX MsgType: ApplicationMessageRequest = "BW"
typedef struct
{
    MessageHeaderInCompT MessageHeaderIn;
...
// Message:     SessionRegistrationResponse
// TemplateID:  10054
// Alias:       Session Registration Response
// FIX MsgType: ApplicationMessageRequestAck = "BX"
typedef struct
{
    MessageHeaderOutCompT MessageHeaderOut;
...
struct OMSResponse {                                //this is for only generate oms response
   public:
    ...
    void SetDefaultValues() {
        header.BodyLen = 0;
        header.TemplateID = 0;
        std::memset(header.Pad2, '\0', LEN_PAD2);
        OrderID = 0;
        ClOrdID = 0;
        OrigClOrdID = 0;
        LeavesQty = 0;
        ...
        FillQty = 0;
        FillExecID = 0;
        std::memset(OrdStatus, 0, sizeof(OrdStatus));
        std::memset(ExecType, 0, sizeof(ExecType));
    }
...
#define	TID_GATEWAYRESPONSE                              10022		// < GatewayResponse (Connection Gateway Response)
...
#define	TID_SESSIONREGISTRATIONREQUEST                   10053		// < SessionRegistrationRequest (Session Registration Request)
#define	TID_SESSIONREGISTRATIONRESPONSE                  10054		// < SessionRegistrationResponse (Session Registration Response)
...
#define LEN_ALLOWTLS                                     1
#define ENUM_ALLOWTLS_CHECKTLS                           "Y"
#define ENUM_ALLOWTLS_SKIPTLS                            "N"
...
#define ENUM_EXECRESTATEMENTREASON_MARKET_ORDER_OUT_OF_RANGE 265
...
#define LEN_INITIALIZATIONVECTOR                         16
...
#define ENUM_QUOTEENTRYREJECTREASON_CLIENT_CODE_NOT_ALLOWED_TO_TRADE_IN_TRADING_RESTRICTION_WINDOW_PIT 155
#define ENUM_QUOTEENTRYREJECTREASON_DEACTIVATED_VIDE_SEBI_CIR_DTD_29_07_22 156
...
#define ENUM_SEGMENTINDICATOR_EGR                        16
