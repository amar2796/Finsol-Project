...
namespace FINSOL
{
	...
	int socket_send(int s, const char* msg, size_t length)
	{
		if (length > 0)
		{
			std::cout <<"[INFO] " << "In Socket Send: " << msg << std::endl;
		}
		int sentBytes = send(s, msg, length, 0);
		if (sentBytes > 0)
		{
			std::cout <<"[INFO] " << "Sent bytes: " << sentBytes << std::endl;
		}
		return sentBytes;
	}
