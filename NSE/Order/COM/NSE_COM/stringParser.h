#ifndef FIN_STRING_PARSER
#define FIN_STRING_PARSER
...
namespace FIN {
	...
	public:
		std::unordered_map< std::string, std::string > fixed_val = { 
			{Fix_BeginString, Fix_version}, 
			{Fix_BodyLength, Fix_bodyLength_Val}, 
			{Fix_MsgSeqNum,Fix_Zero}, 
			{Fix_AvgPX, Fix_Zero}, 
			{Fix_MsgType, Fix_ExcutionReport}, 
			{Fix_SenderCompID, Fix_DefaultValue},
			{Fix_ExecID,Fix_DefaultZero} };
		...
		inline void getOmsString(std::string& out)
		{
			...
			updateString(out, Fix_TriggerPrice);
			updateString(out, "81");
			insertField("10", "000");
			updateString(out, "10");
		}
