...
namespace FIN {
	namespace NSE {
		namespace TRIMMED {
			...
			Connection::Connection(const std::string& fileName, ConfigParameter* configDetail)
				: ConnectionTemplate(configDetail)
			{
				...
				_heartBeatMsg_ = new HEARTBEAT_MESSAGE(configDetail->trader_id);
				_connected = false;
			...
			void Connection::start()
			{
				while (1)
				{
					try
					{
						while (!ConnectSSLSocket())
						{
							std::cout << "SSL Connection not stable, reconnecting after 5 seconds" << std::endl;
							LOG_INFO << "SSL Connection not stable, reconnecting after 5 seconds";
			...
			void Connection::stopSocketConnection()
			{
			...
			void Connection::connectSocket(std::string ip, int port)
			{
				//initilize
				WSADATA wsaData;
				...
				//inet_pton(AF_INET, _config->connectHost.c_str(), &serverAddr.sin_addr);
				...
				std::cout << "Connected to server at " << _config->connectHost << ":" << _config->connectPort << "\n";
			}
			...
			bool Connection::sendToTarget(DC_PACKET_WRAPPER& request)
			{
				...
				if (sendMessage(reinterpret_cast<const char*>(&request), request.getLength()))
				{
					printMessages((char*)&request + sizeof(DC_PACKET_WRAPPER));
					return true;
				}
			...
			bool Connection::sendMessage(const char* data, size_t length)
			{
				return send(clientSocket, data, (int)length, 0) != SOCKET_ERROR;
			}
			...
			bool Connection::sendToTarget_Encrypt(DC_PACKET_WRAPPER& request, bool isBoxLogin)
			{
				...
				if (sendMessage(reinterpret_cast<const char*>(buffer.data()), buffer.size()))
				{
					printMessages((char*)&request + sizeof(DC_PACKET_WRAPPER));
					return true;
				}
			...
			bool Connection::receiveFromTarget()
			{

				memset(_data, 0, 65535);
				int dataRcvd = recv(clientSocket, _data, 65535, 0);		//0 = no special behavior
				//LOG_INFO << "RCVED PACKET " << dataRcvd;


				if (dataRcvd == -1) {
					int errorCode = WSAGetLastError();
					LOG_INFO << "Socket receive failed. Error Code: " << errorCode;
					if (errorCode == WSAECONNRESET || errorCode == WSAENETRESET) {
						closesocket(clientSocket);
				...
				while (pos < dataRcvd)
				{
					...
					if (length <= 0)
					{
						...
						dataRcvd = leftbyte;
						dataRcvd += recv(clientSocket, _data + leftbyte, 65535 - leftbyte, 0);
						pos = 0;
					...
					else if (pos + length > dataRcvd)
					{
						...
						dataRcvd = dataRcvd - pos;
						dataRcvd += recv(clientSocket, _data + dataRcvd, needMoreBytes, 0);
						pos = 0;
