...
namespace FIN 
{
	RabbitMqConsumerTBT::RabbitMqConsumerTBT()
	{
		_channel_id = 0;
		_rabbitPort = 0;
	}
	...
	bool RabbitMqConsumerTBT::connectForTokenRead(std::string &rabbit_ip, int rabbit_port, std::string msg)
	{
		_rabbitIp = rabbit_ip;
	...
	void RabbitMqConsumerTBT::intitializeParameterForToken()
	{
		_channel_id = 1;
		_exchangeName = "TBT_TokenList";
		_exchangeType = "direct";
		_routingKey = "NSECOM";
		_queueName = "NSECOM_Token";
	...
	void RabbitMqConsumerTBT::storeToken(std::string token)
	{
